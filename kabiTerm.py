...
def itzulpenaPrestatu(term,analisia):
    p = subprocess.Popen(['flookup -i -x -b -a  foma/kategorizazioaOrokortuta.fst'],stdin=PIPE,stdout=PIPE,shell=True,close_fds=True)
...
def itzulpena(term,analisia):
    ...
    for s in itzultzeko:
        ...
        if "&OrdZurruna" in s:
            ...
            ordZurruna = True
        p1 = subprocess.Popen(['flookup -i -x -b  foma/itzulpena.fst'],stdin=PIPE,stdout=PIPE,shell=True)
        ...
        for itzul in returnwords[0].decode('utf8').split('\n'):
            ...
            p2 = subprocess.Popen(['flookup -i -x -b  foma/deklinabideak.fst'],stdin=PIPE,stdout=PIPE,shell=True)
